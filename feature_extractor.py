...
def extract_features(image_path):
    ...
    color_hist = _color_histogram(pixels)        # 64 numbers
    edge_density = _edge_density(pixels, width, height)  # 1 number
    dominant_color = _dominant_color(pixels)      # 3 numbers (R, G, B)
    aspect_ratio = _aspect_ratio(image_path)      # 1 number
    
    # Step 4: Combine all features into one list
    features = color_hist + [edge_density] + dominant_color + [aspect_ratio]
