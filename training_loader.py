...
def load_training_data(training_dir, classifier):
    ...
    MAX_IMAGES_PER_CATEGORY = 30  # Cloud memory safety limit
    for category in sorted(categories):
        ...
        for filename in os.listdir(category_path):
            if image_count >= MAX_IMAGES_PER_CATEGORY:
                print(f"  [Memory Limit] Stopping at {MAX_IMAGES_PER_CATEGORY} images for {category}/")
                break
            ...
            try:
                ...
                # Force memory cleanup of large image data immediately
                gc.collect()
